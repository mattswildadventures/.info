...
export default function MobileDropdownNav({
  ...
  const currentCategory = title ? Object.entries(roadmap).find(([_, items]) => 
  ...
  const currentItem = title ? Object.values(roadmap)
    .flat()
  ...
  return (
    <Box ref={dropdownRef} sx={{ position: "relative", mb: 3 }}>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Box sx={{ flex: 1, pr: 2 }}>
            ...
            >
              {currentCategory || "Select Timeline"}
            </Text>
            <Text
              sx={{
                fontSize: "16px",
      ...
      {isOpen && (
        ...
        >
          {Object.entries(roadmap).map(([category, items], categoryIndex) => (
            <Box key={category}>
              <Box
                sx={{
              ...
              {categoryIndex < Object.entries(roadmap).length - 1 && (
                <Box sx={{ 
                  height: "1px", 
