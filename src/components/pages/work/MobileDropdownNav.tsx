...
export default function MobileDropdownNav({
  ...
  const currentCategory = title ? Object.entries(work).find(([_, items]) => 
  ...
  useEffect(() => {
    if (!title) {
      const categories = Object.keys(work);
      if (categories.length > 0) {
        setActiveCategory(categories[0]);
        if (work[categories[0]]?.length > 0) {
          onNavigate?.(work[categories[0]][0].title);
        }
      }
    }
  }, [title, onNavigate]);
  ...
  const currentItem = title ? Object.values(work)
    .flat()
  ...
  return (
    <Box ref={dropdownRef} sx={{ position: "relative", mb: 3 }}>
      ...
      {isOpen && (
        ...
        >
          {Object.entries(work).map(([category, items], categoryIndex) => (
            <Box key={category}>
              <Box
                sx={{
              ...
              {categoryIndex < Object.entries(work).length - 1 && (
                <Box sx={{ 
                  height: "1px", 
